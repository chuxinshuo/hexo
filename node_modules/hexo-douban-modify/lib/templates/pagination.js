...
function makePageNum(num, arr) {
    return (num + 1) + ' / ' + (Math.ceil(arr.length / 30 === 0 ? 1 : Math.ceil(arr.length / 30)));
}
...
function nextBtn() {
    ...
    currNum = currNum.substr(0, currNum.indexOf('/') - 1);
    currNum = parseInt(currNum, 30) - 1;
    if (currNum < Math.ceil(sibs.length / 30) - 1) {
        currNum++;
    }
    displayPage(sibs, currNum);
    this.parentNode.getElementsByClassName('hexo-douban-pagenum')[0].innerText = makePageNum(currNum, sibs);
...
function lastBtn() {
    var sibs = this.parentNode.siblings();
    displayPage(sibs, Math.ceil(sibs.length / 30) - 1);
    this.parentNode.getElementsByClassName('hexo-douban-pagenum')[0].innerText = makePageNum(Math.ceil(sibs.length / 30) - 1 === -1 ? 0 : Math.ceil(sibs.length /30) - 1, sibs);
}

function displayPage(arr, num) {
    for (var i = 0; i < arr.length; i++) {
        if (Math.floor(i / 30) === num) {
...
for (var i = 0; i < firstpages.length; i++) {
    ...
    pagenums[i].innerText = '1 / ' + (Math.ceil(size / 30) === 0 ? 1 : Math.ceil(size / 30));
    firstpages[i].click();
}
